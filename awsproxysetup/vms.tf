resource "aws_security_group" "private-vms" {
...
resource "aws_instance" "private_vms" {
  ...
  vpc_security_group_ids      = [aws_security_group.private-vms.id]
  subnet_id                   = aws_subnet.eu-central-1a-private.id
  associate_public_ip_address = true
  source_dest_check           = false
  private_ip                  = "10.0.1.${count.index + 200}"

  user_data = templatefile("userdata/vms.sh", {
  ...
  tags = {
    Name    = "bhofmann-vpc-vm-${count.index}"
    Creator = "bhofmann"
  }
}

resource "aws_instance" "downstream_vms" {
  count = 3
  ami   = data.aws_ami.ubuntu.id
  ...
  associate_public_ip_address = true
  source_dest_check           = false
  private_ip                  = "10.0.1.${count.index + 210}"
  ...
  tags = {
    Name    = "bhofmann-downstream-vm-${count.index}"
    Creator = "bhofmann"
  }
}