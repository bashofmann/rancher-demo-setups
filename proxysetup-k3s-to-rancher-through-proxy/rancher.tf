...
resource "aws_instance" "rancher_server" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "helm --kubeconfig /etc/rancher/k3s/k3s.yaml upgrade --install rancher rancher-latest/rancher --namespace cattle-system --version 2.5.7 --set hostname=${join(".", ["rancher", self.public_ip, "xip.io"])} --create-namespace || true",
